...
class Insight extends StatefulWidget {
  ...
  const Insight(this.userData, Map<DateTime, Map<String, int>> skinData, {super.key});
...
class _InsightState extends State<Insight> {
  final List<String> items = ["수분", "모공", "여드름", "주름", "색소침착", "탄력"];
      ...
      if (response.statusCode == 200) {
        ...
        (responseData['statistics'] as Map<String, dynamic>)
          ...
          final translatedData =
              value.map((k, v) => MapEntry(keyTranslation[k] ?? k, v)); // 키 변환
                                      ...
                                      rightTitles: const AxisTitles(
                                        sideTitles: SideTitles(
                                          ...
                                          borderData: FlBorderData(
                                              show: false), // 테두리 제거
                                          lineBarsData: skinData.isNotEmpty
                                              ? [
                                                  LineChartBarData(
                                                    spots: List.generate(
                                                      dates.length,
                                                      (index) {
                                                        final value = skinData[
                                                                    dates[
                                                                        index]]
                                                                ?[
                                                                selectedItem] ??
  ...
  void updateLatestData() {
    if (skinData.isNotEmpty) {
      if (skinData.keys.length == 1) {
        // 데이터가 1개일 경우
        latestDate = skinData.keys.first;
      } else {
        // 데이터가 여러 개일 경우
      ...
      latestMoistureScore = 0;
      dot = AppColors.negativePoint;
    }
  }
}
